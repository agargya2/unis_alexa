                ...
                console.log(`INTENT REQUEST`);
                    switch(event.request.intent.name){
                        ...
                        case "GetWeeklySchedule":
                            ...
                            var endpoint = "http://m.uploadedit.com/bbtc/151335577848.txt";
                            var body = "";
                            ...
                            http.get(endpoint, (response) => {
                                ...
                                response.on('end', () => {
                                    ...
                                    for(var j = 0; j < gamesThisWeek.length; j++){
                                        ...
                                        if(requestedGame.tournament === "yes"){
                                            if(requestedGame.teams === "varsity"){
                                                response += "there is a varsity tournament ";
                                            } else if(requestedGame.teams === "both"){
                                                response += "there is a jv and varsity tournamet ";
                                            } else{
                                                response += "there is a jv tournament ";
                                            }
                                            
                                            if(requestedGame.timeStart === "TBA" && requestedGame.opponent == "TBA"){
                                                response += "The opponent and time have not been decided yet. ";
                                            } else if(requestedGame.timeStart === "TBA"){
                                                response += "The game starts at " + requestedGame.timeStart + ", but the opponent has not been decided.";
                                            } else if(requestedGame.timeStart === "TBA"){
                                                response += "The game is against " + requestedGame.opponent + ", but the start of the game has not been decided.";
                                            } else{
                                                response += "The game is against " + requestedGame.opponent + " and it starts at " + requestedGame.timeStart;
                                            }
                                        } else{
                                            if(requestedGame.teams === "varsity"){
                                                response += "there is a varsity game ";
                                            } else if(requestedGame.teams === "both"){
                                                response += "there is a jv and varsity game ";
                                            } else{
                                                response += "there is a jv game ";
                                            }
                                            ...
                                            if(requestedGame.timeStart === "TBA" && requestedGame.opponent == "TBA"){
                                                ...
                                                if(requestedGame.location === requestedGame.opponent){
                                                    response += ". The game is against " + requestedGame.opponent + " at their court and it starts at " + requestedGame.timeStart + ". ";
                                                } else{
                                                    response += "at " + requestedGame.location + ". ";
                                                    response += "The game is against " + requestedGame.opponent + " and it starts at " + requestedGame.timeStart + ". ";
                                                }
                                    ...
                                    context.succeed(
                                        generateResponse(
                                            buildSpeechletResponseWithCard(response, true, "Basketball Game", response), {}
                                        )
                                    );
                                });
                            });
                        ...
                        case "GetScheduleOfDate":
                            ...
                            var endpoint = "http://m.uploadedit.com/bbtc/151335577848.txt";
                            // var endpoint = "http://m.uploadedit.com/bbtc/1506207867385.txt";
                            var body = "";
                            http.get(endpoint, (response) => {
                                response.on('data', (chunk) => { body += chunk });
                                response.on('end', () => {
                                    ...
                                    if(gameNumber === false){
                                        context.succeed(
                                            generateResponse(
                                                buildSpeechletResponse("You don't have a game, but you have practice at Kenny.", true), {}
                                            )
                                        );
                                    } else{
